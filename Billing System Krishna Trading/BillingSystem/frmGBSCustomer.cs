...
namespace BillingSystem
{
    public partial class frmGBSCustomer : Form
    {
        ...
        private void Reset()
        {
            ...
            txt_name.Text = txt_address.Text = txt_city.Text = txt_mobile_no.Text = txt_phone_no.Text = txt_email_id.Text = txt_pan_no.Text = txt_cst_no.Text = txt_tin_no.Text = "";
        ...
        private void BindGrid()
        {
            ...
            sqlString = "SELECT um.CustomerID, um.CustomerName,um.Address, um.City, um.MobileNumber, um.PhoneNumber, um.EmailAddress, um.PanNumber, um.CSTNumber, um.TinNumber FROM Customer AS um where um.IsDeleted = 0";
        ...
        private void FillData()
        {
            try
            {
                if (dgv_master.SelectedRows.Count > 0)
                {
                    ...
                    txt_pan_no.Text = dgv_master.CurrentRow.Cells["PanNumber"].Value.ToString();
                    txt_cst_no.Text = dgv_master.CurrentRow.Cells["CSTNumber"].Value.ToString();
                    txt_tin_no.Text = dgv_master.CurrentRow.Cells["TinNumber"].Value.ToString();
                    txt_name.Focus();
        ...
        private void DeleteRecord()
        {
            try
            {
                if (MessageBox.Show("Are you sure to delete this record ?", "Exit", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    query = "update Customer set IsDeleted = 1 where CustomerID = " + lbl_id.Text.Trim();
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (btn_save.Text == "SAVE")
                query = "INSERT INTO Customer (CustomerName, Address, City,  MobileNumber, PhoneNumber,  EmailAddress, PanNumber, CSTNumber, TinNumber) values ('" + txt_name.Text.Trim() + "','" + txt_address.Text.Trim() + "', '" + txt_city.Text.ToString() + "','" + txt_mobile_no.Text.Trim() + "','" + txt_phone_no.Text.Trim() + "','" + txt_email_id.Text.Trim() + "', '" + txt_pan_no.Text.Trim() + "', '" + txt_cst_no.Text.Trim() + "', '" + txt_tin_no.Text.Trim() + "')";
            else
                query = "UPDATE Customer SET CustomerName = '" + txt_name.Text.Trim() + "', Address = '" + txt_address.Text.Trim() + "', City = '" + txt_city.Text.ToString() + "', MobileNumber = '" + txt_mobile_no.Text.Trim() + "', PhoneNumber = '" + txt_phone_no.Text.Trim() + "', EmailAddress = '" + txt_email_id.Text.Trim() + "', PanNumber = '" + txt_pan_no.Text.Trim() + "', CSTNumber = '" + txt_cst_no.Text.Trim() + "', TinNumber = '" + txt_tin_no.Text.Trim() + "' where CustomerID = " + lbl_id.Text.Trim();
            string message = _datalayer.Opration(query);
            if (message == "Success")
            {
        ...
        private void dgv_master_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            dgv_master.Columns["PanNumber"].HeaderText = "Pan Number";
            dgv_master.Columns["CSTNumber"].HeaderText = "CST Number";
            dgv_master.Columns["TinNumber"].HeaderText = "Tin Number";
        ...
        private void txt_search_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                (dgv_master.DataSource as DataTable).DefaultView.RowFilter = string.Format("CustomerName LIKE '%{0}%' OR Address LIKE '%{0}%' OR City LIKE '%{0}%' OR MobileNumber LIKE '%{0}%' OR PhoneNumber LIKE '%{0}%' OR EmailAddress LIKE '%{0}%' OR PanNumber LIKE '%{0}%' OR CSTNumber LIKE '%{0}%' OR TinNumber LIKE '%{0}%'", txt_search.Text);
                dgv_master.Refresh();
            }
        }
        ...
        private void txt_cst_no_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                e.SuppressKeyPress = true;
        ...
        private void txt_tin_no_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                e.SuppressKeyPress = true;
