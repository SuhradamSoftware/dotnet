...
namespace BillingSystem
{
    public partial class frm_Tax_master : Form
    {
        ...
        public void BindGrid()
        {
            ...
            query = "SELECT um.fld_tax_id, um.fld_tax_description, um.fld_tax_rate FROM tbl_taxes AS um where um.fld_is_deleted = 0";
        ...
        public void FillData()
        {
            try
            {
                ...
                {
                    lbl_id.Text = dgv_master.CurrentRow.Cells["fld_tax_id"].Value.ToString();
                    txt_name.Text = dgv_master.CurrentRow.Cells["fld_tax_description"].Value.ToString();
                    txt_Rate.Text = dgv_master.CurrentRow.Cells["fld_tax_rate"].Value.ToString();
        ...
        public void DeleteRecord()
        {
            try
            {
                ...
                {
                    string query = string.Empty;
                    query = "update tbl_taxes set fld_is_deleted = 1, DeletedByID = " + DataAccess.gbl_longUserID + ", DeletedDate = GETDATE() where fld_tax_id = " + lbl_id.Text.Trim();
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                ep1.Clear();
                ep1.SetError(txt_Rate, "Please enter Tax fld_rate.");
                txt_Rate.Focus();
                return;
            }
            ...
            if (btn_save.Text == "Save")
                query = "insert into tbl_taxes (fld_tax_description, fld_tax_rate) values ('" + txt_name.Text.Trim() + "'," + txt_Rate.Text.Trim() + ")";
            else
                query = "update tbl_taxes set fld_tax_description = '" + txt_name.Text.Trim() + "', fld_tax_rate = " + txt_Rate.Text.Trim() + " where fld_tax_id = " + lbl_id.Text.Trim();
            string message = _datalayer.Opration(query);
            if (message == "Success")
            {
        ...
        {
            dgv_master.Columns["fld_tax_id"].Visible = false;
            dgv_master.Columns["fld_tax_description"].HeaderText = "Tax Name";
            dgv_master.Columns["fld_tax_rate"].HeaderText = "fld_tax_rate";
        ...
        private void txt_search_TextChanged(object sender, EventArgs e)
        {
            if (txt_search.Text != "Search...")
            {
                if (dgv_master.DataSource != null)
                {
                    ...
                    {
                        (dgv_master.DataSource as DataTable).DefaultView.RowFilter = string.Format("fld_tax_description LIKE '%{0}%'", txt_search.Text);
                        dgv_master.Refresh();
                    }
                }
            }
        ...
        private void txt_TaxRate_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter && (e.KeyData == Keys.LShiftKey || e.KeyData == Keys.LShiftKey))
            {
                txt_Rate.Text = txt_Rate.Text + "\n";
            }
