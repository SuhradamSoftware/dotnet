    ...
    public partial class frmGBSSetting : Form
    {
        ...
        public void FillNameValue()
        {
            ...
            if (nameValueData.Tables.Count > 0)
            {
                for (int i = 0; i < nameValueData.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        txt_billno.Text = nameValueData.Tables[0].Rows[i]["NameValue"].ToString();
                        lblBillKey.Text = nameValueData.Tables[0].Rows[i]["NameKey"].ToString();
                    }
                    else if (nameValueData.Tables[0].Rows[i]["NameKey"].ToString() == "BillPrefix")
                    {
                        txt_Prefix.Text = nameValueData.Tables[0].Rows[i]["NameValue"].ToString();
                        lbl_Prefix.Text = nameValueData.Tables[0].Rows[i]["NameKey"].ToString();
                    }
                }
            }
        }
        ...
        public void DeleteRecord()
        {
            try
            {
                ...
                {
                    string query = string.Empty;
                    query = "update Taxes set IsDeleted = 1, DeletedByID = " + DataAccess.gbl_longUserID + ", DeletedDate = GETDATE() where TaxID = " + lbl_id.Text.Trim();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (btn_save.Text == "SAVE")
                query = "insert into Taxes (TaxDescription, TaxRate, CreatedByID) values ('" + txt_name.Text.Trim() + "'," + txt_Rate.Text.Trim() + "," + DataAccess.gbl_longUserID + ")";
            else
                query = "update Taxes set TaxDescription = '" + txt_name.Text.Trim() + "', TaxRate = " + txt_Rate.Text.Trim() + ",UpdatedByID = " + DataAccess.gbl_longUserID + ", UpdatedDate = GETDATE() where TaxID = " + lbl_id.Text.Trim();
            string message = _datalayer.Opration(query);
            if (message == "Success")
            {
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            string query = string.Empty;
            query = "UPDATE SSNameValues SET NameValue = '" + txt_billno.Text.Trim() + "' WHERE NameKey = '" + lblBillKey.Text.Trim() + "'";
            string message = _datalayer.Opration(query);
        ...
        private void btn_PrefixUpdate_Click(object sender, EventArgs e)
        {
            ...
            string query = string.Empty;
            query = "UPDATE SSNameValues SET NameValue = '" + txt_Prefix.Text.Trim() + "' WHERE NameKey = '" + lbl_Prefix.Text.Trim() + "'";
            string message = _datalayer.Opration(query);
